...
public class WechatInfoService {
	...
	public PageData findHeadUrlByUserId(PageData pd) throws Exception {
		Map map = new HashMap();
		try {
			map = RedisUtil.getInstance().hgetAll(pd.getString("USER_ID"));
		} catch (Exception e) {
			e.printStackTrace();
			logger.info("出错用户userId--" + pd.getString("USER_ID"));
		}
		if(map.size() == 0 || map.size() < 3) {
			PageData user = (PageData) dao.findForObject("WechatInfoMapper.findHeadUrlByUserId", pd);
			RedisUtil.getInstance().hset(pd.getString("USER_ID"), user);
			return user;
		} else {
